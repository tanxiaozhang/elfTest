...
public class DateUtil {
    ...
    public static final String DATE_TIME_FORMAT_CHN = "yyyy年MM月dd日";
     ...
     * @return 相差天数
     */
    public static Long getDistanceDays(String dateStr1, String dateStr2) throws Exception {
        DateFormat df = new SimpleDateFormat(DATE_TIME_FORMAT_YYYY_MM_DD_HH_MI_SS);
        Date one;
        Date two;
        long days = 0;
        try {
            one = df.parse(dateStr1);
            two = df.parse(dateStr2);
            long time1 = one.getTime();
            long time2 = two.getTime();
            long diff;
            if (time1 < time2) {
